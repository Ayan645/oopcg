...
// This file demonstrates the use of maps for associative containers in C++.
// Theory:
// - **Map**: A container that stores key-value pairs in sorted order by key.
// - **Efficient Lookups**: Maps provide logarithmic time complexity for search, insert, and delete operations.
// - **Iterators**: Used to traverse map elements for displaying all stored data.
// Example: The program stores the population of various states, allows users to retrieve population data by state name, and supports dynamic queries.
